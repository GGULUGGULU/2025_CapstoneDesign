...
void CMaterial::UpdateShaderVariable(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	if (m_d3dGpuSrvHandle.ptr != 0)
	{
		pd3dCommandList->SetGraphicsRootDescriptorTable(3, m_d3dGpuSrvHandle);
	}
}
...
void CMaterial::PrepareShaders(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	...
	m_pIlluminatedShader->AddRef(); 

	m_pDiffusedShader = new CDiffusedShader();
...
void CGameObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CGameObject* pDebugBox, CCamera* pCamera)
{
	...
	if (IsVisible(pCamera))
	{
		...
		if (pDebugBox && m_pMesh)
		{
			BoundingBox localAABB = m_pMesh->GetBoundingBox();
			...
			XMMATRIX matThisWorld = XMLoadFloat4x4(&m_xmf4x4World);

			XMMATRIX matFinalWorld = matScale * matTranslate * matThisWorld;

			XMStoreFloat4x4(&pDebugBox->m_xmf4x4World, matFinalWorld);

			pDebugBox->UpdateShaderVariable(pd3dCommandList, &pDebugBox->m_xmf4x4World);

			if (pDebugBox->m_ppMaterials[0])
			{
				if (pDebugBox->m_ppMaterials[0]->m_pShader)
					pDebugBox->m_ppMaterials[0]->m_pShader->Render(pd3dCommandList, pCamera);

				pDebugBox->m_ppMaterials[0]->UpdateShaderVariable(pd3dCommandList);
			}

			if (pDebugBox->m_pMesh)
				pDebugBox->m_pMesh->Render(pd3dCommandList);
		}
	}

	if (m_pSibling) m_pSibling->Render(pd3dCommandList, pDebugBox, pCamera);
	if (m_pChild) m_pChild->Render(pd3dCommandList, pDebugBox, pCamera);
}

void CGameObject::ComputeCombinedAABB()
{
	m_xmCombinedLocalAABB = BoundingBox();

	if (m_pMesh)
	{
		m_xmCombinedLocalAABB = m_pMesh->GetBoundingBox();
	}
	...
	while (pChild)
	{
		pChild->ComputeCombinedAABB();

		BoundingBox childAABB = pChild->GetCombinedAABB();

		BoundingBox childTransformedAABB;
		childAABB.Transform(childTransformedAABB, XMLoadFloat4x4(&pChild->m_xmf4x4Transform));

		BoundingBox::CreateMerged(m_xmCombinedLocalAABB, m_xmCombinedLocalAABB, childTransformedAABB);
...
CGameObject* CGameObject::PickObject(XMVECTOR& vWorldRayOrigin, XMVECTOR& vWorldRayTarget, float& fMinDistance)
{
	CGameObject* pSelectedObject = NULL; 

	if (m_pMesh)
	{
		XMMATRIX matInvWorld = XMMatrixInverse(NULL, XMLoadFloat4x4(&m_xmf4x4World));
		...
		float fHitDistance;
		if (m_pMesh->CheckRayIntersection(fLocalOrigin, fLocalDir, &fHitDistance))
		{
			if (fHitDistance < fMinDistance)
			{
				fMinDistance = fHitDistance; 
	...
	if (m_pChild)
	{
		CGameObject* pClosestChild = m_pChild->PickObject(vWorldRayOrigin, vWorldRayTarget, fMinDistance);
		if (pClosestChild)
		{
			pSelectedObject = pClosestChild;
		}
	}

	if (m_pSibling)
	{
		CGameObject* pClosestSibling = m_pSibling->PickObject(vWorldRayOrigin, vWorldRayTarget, fMinDistance);
		if (pClosestSibling)
		{
			pSelectedObject = pClosestSibling;
		}
	}
	...
	for ( ; ; )
	{
		...
		else if (!strcmp(pstrToken, "<Mesh>:"))
		{
			...
			if (pMeshInfo)
			{
				...
				///// Ãß°¡
				else if (pMeshInfo->m_nType & VERTEXT_POSITION) 
				{
		...
		else if (!strcmp(pstrToken, "<Materials>:"))
		{
			...
			if (pMaterialsInfo && (pMaterialsInfo->m_nMaterials > 0))
			{
				...
				for (int i = 0; i < pMaterialsInfo->m_nMaterials; i++)
				{
					...
					else pMaterial->SetShader(CMaterial::m_pDiffusedShader);
...
void CRotatingObject::Render(ID3D12GraphicsCommandList *pd3dCommandList,  CGameObject* pDebugBox, CCamera *pCamera)
{
	CGameObject::Render(pd3dCommandList, pDebugBox, pCamera);
}
...
///////////////////////////////////////////////////////////////////
CParticleEmitter::CParticleEmitter(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int nMaxParticles)
{
	...
	for (int i = 0; i < m_nMaxParticles; ++i)
	{
		...
		m_vCpuParticles[i].m_fAge = 999.0f;
	}

	UINT nParticleStride = sizeof(ParticleRenderData);
	UINT nBufferSize = nParticleStride * nMaxParticles;

	m_pd3dDynamicVertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, nBufferSize,
		D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, NULL);

	m_pd3dDynamicVertexBuffer->Map(0, NULL, (void**)&m_pMappedParticles);

	m_d3dVertexBufferView.BufferLocation = m_pd3dDynamicVertexBuffer->GetGPUVirtualAddress();
	m_d3dVertexBufferView.StrideInBytes = nParticleStride;
	m_d3dVertexBufferView.SizeInBytes = nBufferSize;
}

CParticleEmitter::~CParticleEmitter()
{
	if (m_pd3dDynamicVertexBuffer)
	{
		m_pd3dDynamicVertexBuffer->Unmap(0, NULL);
		m_pd3dDynamicVertexBuffer->Release();
	}
}

void CParticleEmitter::SpawnExplosion(XMFLOAT3 xmf3Position)
{
	int nSpawnCount = 0;
	for (int i = 0; i < m_nMaxParticles; ++i)
	{
		if (m_vCpuParticles[i].m_bIsActive == false)
		{
			m_vCpuParticles[i].m_bIsActive = true;
			m_vCpuParticles[i].m_fAge = 0.0f;
			m_vCpuParticles[i].m_fLifeTime = 3.0f; 
			m_vCpuParticles[i].m_xmf3Position = xmf3Position;
			m_vCpuParticles[i].m_xmf2MaxSize = XMFLOAT2(15.0f, 15.0f);

			float x = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
			float y = ((float)rand() / RAND_MAX) * 1.0f; 
			float z = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
			m_vCpuParticles[i].m_xmf3Velocity = Vector3::Normalize(XMFLOAT3(x, y, z));
			m_vCpuParticles[i].m_xmf3Velocity.x = m_vCpuParticles[i].m_xmf3Velocity.x * 30.0f; 
			m_vCpuParticles[i].m_xmf3Velocity.y = m_vCpuParticles[i].m_xmf3Velocity.y * 30.0f; 
			m_vCpuParticles[i].m_xmf3Velocity.z = m_vCpuParticles[i].m_xmf3Velocity.z * 30.0f; 

			nSpawnCount++;
			if (nSpawnCount > 50) break; 
		}
	}
}

void CParticleEmitter::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	...
	for (int i = 0; i < m_nMaxParticles; ++i)
	{
		if (m_vCpuParticles[i].m_bIsActive)
		{
			...
			{
				m_vCpuParticles[i].m_xmf3Velocity.y -= 9.8f * fTimeElapsed; 
				m_vCpuParticles[i].m_xmf3Position = Vector3::Add(m_vCpuParticles[i].m_xmf3Position, m_vCpuParticles[i].m_xmf3Velocity, fTimeElapsed);

				float age = m_vCpuParticles[i].m_fAge;
				float lifeTime = m_vCpuParticles[i].m_fLifeTime;
				XMFLOAT2 maxSize = m_vCpuParticles[i].m_xmf2MaxSize;
				XMFLOAT2 currentSize;

				const float peakTime = 1.0f;
				float scale = 0.0f;
				...
				{
					float shrinkDuration = lifeTime - peakTime;
					...
					{
						float shrinkProgress = (age - peakTime) / shrinkDuration;
						scale = 1.0f - (std::min)(shrinkProgress, 1.0f);
					}
				}

				currentSize.x = maxSize.x * scale;
				currentSize.y = maxSize.y * scale;
...
void CParticleEmitter::Render(ID3D12GraphicsCommandList* pd3dCommandList, CGameObject* pDebugBox, CCamera* pCamera)
{
	if (m_nActiveParticles == 0) return; 

...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	if ((nCurrentCameraMode == FIRST_PERSON_CAMERA) || (nCurrentCameraMode == THIRD_PERSON_CAMERA))//|| (nCurrentCameraMode == SPACESHIP_CAMERA))
	{
		if (x != 0.0f)
		{
			m_fPitch += x;
...
void CPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CGameObject* pDebugBox, CCamera* pCamera)
{
	DWORD nCameraMode = (pCamera) ? pCamera->GetMode() : 0x00;
	if (!pCamera || nCameraMode == THIRD_PERSON_CAMERA || nCameraMode == FIRST_PERSON_CAMERA)
		// nCameraMode == FIRST_PERSON_CAMERA가 없으면 그려지지 않아 충돌판정이 안됨
	{
